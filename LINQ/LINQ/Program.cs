...
namespace LINQ
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Readjusted to use value tuples instead of tuple
            var maxNumberNonPoisonedRedApplesSuccessive = apples.Aggregate( (0, 0), (acc, apple) =>
                  (apple.Colour == "Red" && !apple.Poisoned)
                  ?  (acc.Item1 + 1, Math.Max(acc.Item1 + 1, acc.Item2))
                  :  (0, Math.Max(acc.Item1, acc.Item2))).Item2;
